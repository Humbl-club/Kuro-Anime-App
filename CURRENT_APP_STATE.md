...
- 2026-02-08: **Negative genre mode suppression** — `mapStrongGenreToModeId()` now accepts excluded genres and skips conflicting mode matches (e.g. "action but no romance" no longer routes to romcom). `scoreMode()` penalizes modes whose required_genres overlap with excluded genres. Deployed as concierge-recommend v30. Router eval hardened: exponential backoff for 429/5xx, infra errors separated from routing failures in pass rate.
