-- Realm Graph Stage 1 acceptance harden: Spirited Away test.
-- Diagnosis (live 2026-08-02):
--   Re:ZERO S4 passed via SHARED grand-adventure @ 0.30 (seed S included it at T=0.25).
--   JJK passed via SHARED supernatural-yokai @ 0.50 (top realm battle-shounen).
-- Fix:
--   1) Canon seeds: seed_realms + shared-pass use weight >= 0.35.
--   2) Canon seeds: shared pass also requires candidate top realm ∈ S.
--   3) Affinity veto: battle-shounen ↔ supernatural-yokai (measured 0.93 highway).

begin;

insert into public.realm_affinity_overrides (realm_a, realm_b, affinity, reason) values
  ('battle-shounen', 'supernatural-yokai', 0.0,
   'Spirited Away test: yokai folklore must not affinity-admit battle shounen (measured 0.93)'),
  ('supernatural-yokai', 'battle-shounen', 0.0,
   'Spirited Away test: yokai folklore must not affinity-admit battle shounen (measured 0.93)'),
  ('isekai-reincarnation', 'auteur-cinema', 0.0,
   'Spirited Away test: auteur film neighborhood must not affinity-admit LN isekai'),
  ('auteur-cinema', 'isekai-reincarnation', 0.0,
   'Spirited Away test: auteur film neighborhood must not affinity-admit LN isekai')
on conflict (realm_a, realm_b) do update
