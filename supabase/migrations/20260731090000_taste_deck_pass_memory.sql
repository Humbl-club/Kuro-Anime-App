-- PASS MEMORY 2026-07-31: deck passes become durable and neutral.
-- Apply after 20260731080000. Four recreated functions, nothing else.
--
-- Bug: PASS recorded nothing, so passed titles stayed eligible and were
-- re-dealt (live-verified: 2/12 of batch 2 were batch-1 passes). A pass is not
-- a dislike — it is "dealt, undecided" — so it must be remembered WITHOUT
-- moving the taste profile:
--   record_taste_deck_signal   new action 'pass' -> event_type 'deck_pass',
--                              event_strength 0.00, signal_value {"action":"pass"};
--                              same one-signal-per-user/media replace semantics;
--                              passes count toward the 300/day rate budget.
--                              Recompute enqueue: a PURE pass alters nothing in
--                              the profile (strength 0, excluded from recompute)
--                              so it does NOT enqueue; a pass that REPLACES a
--                              scored signal (love/known/skip) MUST enqueue so
--                              the profile drops that title's contribution.
--                              retract path unchanged.
--   fetch_taste_deck_batch     byte-identical to fix2 except the prior-signal
--                              exclusion lists gain 'deck_pass' (passed titles
--                              are never re-dealt).
--   recompute_user_taste_profile  byte-identical to fix2 except deck_pass rows
--                              are excluded (a) from the evidence/event counts
--                              and (b) from the event aggregation CTE (strength
--                              0 is harmless to the sums but wastes per-title
--                              cap bookkeeping — excluded at the source).
--   _taste_deck_apply_tag_stats   explicit 'pass' guard: no alpha/beta deltas
--                              (the zero-delta early return already covered it;
--                              the guard makes the contract explicit).
-- No profile re-enqueue on apply: deck_pass rows cannot exist before this
-- migration, so existing profiles are already consistent with these rules.

-- ---------------------------------------------------------------------------
-- 1) _taste_deck_apply_tag_stats: explicit pass guard (060000 form + guard)
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- 'pass' is durable memory, not a vote: no alpha/beta movement either way
  -- (apply or reversal). Redundant with the zero-delta guard below on purpose.
...
-- ---------------------------------------------------------------------------
-- 2) record_taste_deck_signal: accept 'pass' (070000 form + pass semantics)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3) fetch_taste_deck_batch: byte-identical to fix2 except the two
--    prior-signal exclusion lists gain 'deck_pass'.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4) recompute_user_taste_profile: byte-identical to fix2 except deck_pass is
--    excluded from (a) the evidence/event counts and (b) the event CTE.
-- ---------------------------------------------------------------------------
