-- TEMPORARY debug #5: exact replica of the live recommend_ids_similar_to_seeds body
-- under a fresh name, to isolate stale-plan vs data explanations. Dropped after diagnosis.
drop function if exists public.recommend_ids_similar_to_seeds_v42(text, integer[], integer, boolean);
create function public.recommend_ids_similar_to_seeds_v42(p_media_type text, p_seed_ids integer[], p_limit integer default 10, p_allow_gimmicks boolean default false)
...
$function$;
revoke all on function public.recommend_ids_similar_to_seeds_v42(text, integer[], integer, boolean) from public;
grant execute on function public.recommend_ids_similar_to_seeds_v42(text, integer[], integer, boolean) to anon, authenticated;
