-- Entry-point upward tier substitution (20260805150000).
--
-- WHY: the 20260805140000 entry-point mapping gated the entry on the SAME
--   tier-distance rule as candidates (|entry_tier - seed_tier| <= 1). Verified
--   on the rebuilt gold stores: franchise ENTRY POINTS skew canon-tier, so
--   mid-tier seeds systematically could NOT receive them and kept the
--   mid-franchise member — the exact failure the fix targets. Both remaining
--   evidenced failures reduce to it on live data:
--     * Tokyo Revengers (124, tier solid): "MHA: Two Heroes" (301, acclaimed,
--       passes distance 1) stayed at rank 19 because entry MHA Season 1 (6)
--       is canon — distance 2, gated.
--     * Same rail rank 10: "WIND BREAKER Season 2" (1076, solid) kept because
--       entry WIND BREAKER (430) is canon — distance 2, gated.
--
-- FIX (minimal diff on the 20260805140000 builder body — the four mapping
--   CASE gates only, plus a candidate-tier join in the two Stage-B mapping
--   subqueries):
--     entry tier gate becomes
--       within seed distance 1  OR  entry_tier >= candidate_tier.
--   Substituting a HIGHER-OR-EQUAL-tier member of the SAME franchise is never
--   a quality regression — the franchise already earned the slot through a
--   candidate that passed the stage's tier gate; the entry is just which
--   member is served. DOWNWARD substitution (entry tier below the candidate)
--   still requires the entry to sit inside the seed's tier neighborhood, and
--   the canon-seed merit floor and pool gates stay hard — so
--   Berserk (canon) -> Steel Ball Run keeps NOT remapping to JoJo Part 1
--   (solid, avg 70): tier blocked (2 < 4, distance 2) AND merit blocked.
--
-- Everything else is byte-identical to 20260805140000 (which was built as a
-- verified byte-exact minimal diff on the live 20260805120000 body).
--
-- REVERT PATH: restore the 20260805140000 builder body (that file's section
--   2) and re-stale; or restore 20260805120000 to drop entry mapping wholly.
