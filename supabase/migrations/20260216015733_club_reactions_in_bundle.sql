...
-- ============================================================
-- Update fetch_club_bundle to include per-item reactions:
--   reactions: {fire: 3, heart: 1} (aggregate counts, anonymous)
--   my_reactions: ["fire"] (caller's own)
-- Also adds episode_count/chapter_count to items for pace sync.
-- ============================================================
          ...
          SELECT
            ...
            CASE
              ...
              ELSE (
                ...
                FROM (
                  SELECT cm4.user_id, pp.display_name, ul4.list_type, ul4.progress, ul4.updated_at
                  FROM public.club_members cm4
                  LEFT JOIN public.profiles pp ON pp.id = cm4.user_id
                  JOIN LATERAL (
                    SELECT CASE
