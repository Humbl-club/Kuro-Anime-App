  ...
  ranked as (
    select
      *,
      ...
      + case
          when (select tsq from ts) is not null and (select norm from q) is not null
            then coalesce(ts_rank_cd(to_tsvector('simple', title_blob), (select tsq from ts)), 0)::double precision
          else 0::double precision
        end
      + case
          when (select norm_len from q) >= 5 and (select norm from q) is not null
            then greatest(
              extensions.similarity(title_english_norm, (select norm from q)),
              extensions.similarity(title_romaji_norm, (select norm from q)),
              extensions.similarity(title_native_norm, (select norm from q)),
