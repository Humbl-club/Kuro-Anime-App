...
CREATE TABLE IF NOT EXISTS public.media_relation_refresh_queue (
...
CREATE INDEX IF NOT EXISTS idx_media_relation_refresh_queue_status_requested
  ON public.media_relation_refresh_queue (status, requested_at);

ALTER TABLE public.media_relation_refresh_queue ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'set_updated_at')
     AND NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'media_relation_refresh_queue_set_updated_at') THEN
    CREATE TRIGGER media_relation_refresh_queue_set_updated_at
      BEFORE UPDATE ON public.media_relation_refresh_queue
      FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
  END IF;
END;
$$;

CREATE OR REPLACE FUNCTION public.enqueue_media_relation_refresh(
  p_media_type text,
  p_media_id int,
...
BEGIN
  IF normalized_media_type NOT IN ('ANIME', 'MANGA') THEN
    RAISE EXCEPTION 'Unsupported media type for ladder refresh: %', p_media_type
      USING ERRCODE = '22023';
  END IF;

  IF normalized_media_type = 'ANIME' THEN
  ...
  INSERT INTO public.media_relation_refresh_queue (
    media_type,
    media_id,
    reason,
    status,
  ...
  VALUES (
    ...
    p_media_id,
    COALESCE(NULLIF(trim(p_reason), ''), 'on_demand'),
    'pending',
    now(),
    NULL,
    NULL,
...
REVOKE ALL ON FUNCTION public.enqueue_media_relation_refresh(text, int, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.enqueue_media_relation_refresh(text, int, text) TO anon, authenticated, service_role;
