...
REPORT_DIR="${CATALOG_SAFETY_REPORTS_DIR:-$ROOT/reports/catalog-safety}"
mkdir -p "$REPORT_DIR"

export CATALOG_SAFETY_REPORTS_DIR="$REPORT_DIR"
export CATALOG_SAFETY_BATCH_SIZE="${CATALOG_SAFETY_BATCH_SIZE:-200}"
export CATALOG_SAFETY_MEDIA_TYPES="${CATALOG_SAFETY_MEDIA_TYPES:-ANIME,MANGA}"
export CATALOG_SAFETY_OPEN_GAPS_LIMIT="${CATALOG_SAFETY_OPEN_GAPS_LIMIT:-400}"
export CATALOG_SAFETY_MODEL_BLOCKED_CONFIDENCE="${CATALOG_SAFETY_MODEL_BLOCKED_CONFIDENCE:-0.78}"
export CATALOG_SAFETY_MODEL_SAFE_CONFIDENCE="${CATALOG_SAFETY_MODEL_SAFE_CONFIDENCE:-0.70}"

LOG_FILE="$REPORT_DIR/worker.log"
WORKER_BIN="$REPORT_DIR/catalog_safety_worker"
LOCK_DIR="$REPORT_DIR/.worker.lock"
LOCK_PID_FILE="$LOCK_DIR/pid"
LOCK_STARTED_FILE="$LOCK_DIR/started_at_unix"
LOCK_TTL_SECONDS="${CATALOG_SAFETY_LOCK_TTL_SECONDS:-1800}"
...
{
  log_line "starting catalog safety worker"
  if [[ ! -x "$WORKER_BIN" || "$ROOT/scripts/catalog_safety_worker.swift" -nt "$WORKER_BIN" ]]; then
    /usr/bin/xcrun swiftc -parse-as-library "$ROOT/scripts/catalog_safety_worker.swift" -o "$WORKER_BIN"
  fi
  "$WORKER_BIN"
  log_line "catalog safety worker completed"
} >>"$LOG_FILE" 2>&1
